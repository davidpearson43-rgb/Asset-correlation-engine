...
While my first project measured the internal volatility of a single asset, this script analyzes the interaction between *two* assets. By calculating their correlation coefficient, we can mathematically prove if two stocks (like Exxon and Chevron) move in lockstep due to shared macroeconomic factors. This type of analysis is the foundational mathematics behind Pairs Trading and Statistical Arbitrage.
...
* **pandas & numpy** (Vectorized math and matrix operations)
* **matplotlib** (Scatter plots and regression lines)

## 🚀 Usage

The core logic is wrapped in `analyze_asset_correlation()`. Just pass two ticker symbols to generate the correlation matrix and the regression plot.

```python
from correlation_engine import analyze_asset_correlation

# Example: Measure the entanglement of two major energy sector stocks
analyze_asset_correlation("XOM", "CVX", period="1y")
